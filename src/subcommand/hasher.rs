...
pub fn run<P: AsRef<Path>>(repository: NssRepository, path: P) -> Result<()> {
    let object = Object::new(repository.path().join(path.as_ref()))?;
...
pub fn run_option_w<P: AsRef<Path>>(repository: NssRepository, path: P) -> Result<()> {
    let object = Object::new(repository.path().join(path.as_ref()))?;

    let hash = hash_to_hex(object.to_hash());
    file_system::write_object(repository.objects_path(&hash), object)?;
...
fn commit(message: &str) -> std::io::Result<()> {
    ...
    Ok(())
        ...
        let res = run(test_repo.clone(), "first.rs");
        assert!(res.is_ok());

        // Vertfiy blob hash with no existed file
        let res = run(test_repo.clone(), "second.rs");
        assert!(res.is_err());

        // Vertfiy tree hash with existed file
        let res = run(test_repo, temp_dir.clone());
        assert!(res.is_ok());
...
fn commit(message: &str) -> std::io::Result<()> {
    ...
    Ok(())
        ...
        let res = run_option_w(test_repo.clone(), "first.rs");
        assert!(res.is_ok());
...
fn commit(message: &str) -> std::io::Result<()> {
    ...
    Ok(())
        ...
        let res = run_option_w(test_repo.clone(), "second.rs");
        assert!(res.is_err());

        // Vertfiy tree hash with existed file
        let res = run_option_w(test_repo.clone(), "test_sub_dir");
        assert!(res.is_ok());
