...
pub fn run<P: AsRef<Path>>(repo_path: P) -> Result<()> {
    ...
    // Initial File
    create_file_with_buffer(repo_path.join(".nss").join("repo"), repo_path.to_str().unwrap().as_bytes())?;
    create_file_with_buffer(repo_path.join(".nss").join("HEAD"), b"bookmarker: bookmarks/local/voyage")?;
    create_file_with_buffer(repo_path.join(".nss").join("config"), b"remotes: []")?;
    create_file_with_buffer(repo_path.join(".nss").join("INDEX"), b"")?;
    create_file_with_buffer(repo_path.join(".nss").join("bookmarks").join("local").join("voyage"), b"")?;
...
mod tests {
    ...
    fn test_run() {
        ...
        assert!(temp_dir.join(".nss").join("bookmarks").join("local").is_dir());
        ...
        assert!(temp_dir.join(".nss").join("bookmarks").join("local").join("voyage").is_file());
